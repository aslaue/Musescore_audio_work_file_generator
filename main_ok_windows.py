...
def convert_to_musicxml(path_and_name_mscz):
    ...
    if 'win' in sys.platform: # windows
        default_MS_path = "C:\\Program Files\\MuseScore 3\\bin\\Musescore.exe"

        if not os.path.exists(default_MS_path):
            # "C:\Appportables\MuseScorePortableLegacy3\MuseScorePortable.exe"
            if os.path.exists("C:\\Appportables\\MuseScorePortableLegacy3\\MuseScorePortable.exe"):
                default_MS_path = "C:\\Appportables\\MuseScorePortableLegacy3\\MuseScorePortable.exe"
            # elif os.path.exists("C:\\Program Files\\MuseScore 4\\bin\\Musescore4.exe"):
                # default_MS_path = "C:\\Program Files\\MuseScore 4\\bin\\Musescore4.exe"
            else:
                messagebox.showinfo("Alerte", "Fichier Musescore.exe non trouvé, veuillez indiquer l'emplacement du fichier")
                default_MS_path = str(askopenfile(mode ='r', filetypes =[('Executable File', '.exe')], title='Sélectionner le fichier de Musescore')).split('name=\'')[1].split('\'')[0]
...
for i in fichiers_musicxml_crees:
    if sys.platform=="linux":
        ...
        default_MS_path = "C:\\Program Files\\MuseScore 4\\bin\\Musescore4.exe"
        # commande = default_MS_path + ' ' + i + ' -o ' + i.replace(path_dossier_temporaire,path_output) + ".mp3\n"
